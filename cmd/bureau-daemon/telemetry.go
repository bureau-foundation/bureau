...
// generateLogSessionID produces a session ID for output capture log
// correlation. The format is "<localpart>-<unixNano>", which the
// telemetry service uses to build log tag names.
func (d *Daemon) generateLogSessionID(localpart string) string {
	return fmt.Sprintf("%s-%d", localpart, d.clock.Now().UnixNano())
}

// configureLogForPrincipal tells the telemetry service to use a
// per-source eviction limit for this principal. Called after sandbox
// creation when the template specifies OutputCapture.MaxSize > 0.
//
// Best-effort: logs errors but never returns them. The telemetry
// service falls back to its global default if this call fails — the
// principal's output is still captured, just with the default limit.
func (d *Daemon) configureLogForPrincipal(ctx context.Context, principal ref.Entity, maxBytes int64) {
	if d.telemetrySocketPath == "" {
		return
	}
	...
	if err != nil {
		d.logger.Error("failed to mint configure-log token",
			"principal", principal,
			"error", err,
		)
		return
	...
	request := telemetry.ConfigureLogRequest{
	...
	var response telemetry.ConfigureLogResponse
	if err := client.Call(callContext, "configure-log", request, &response); err != nil {
		d.logger.Error("configure-log call failed",
			"principal", principal,
			"max_bytes", maxBytes,
			"error", err,
		)
	...
	d.logger.Info("telemetry log limit configured for principal",
		"principal", principal,
		"max_bytes", maxBytes,
	)
}
...
// output and mark the specified log session for this principal as
// complete. The sessionID identifies the specific session to complete.
//
...
func (d *Daemon) completeLogForPrincipal(ctx context.Context, principal ref.Entity, sessionID string) {
	if d.telemetrySocketPath == "" {
		return
	}
