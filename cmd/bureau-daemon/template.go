...
// chain to produce a fully-merged TemplateContent. The returned content has
// all inherited fields merged (base → parent → child) with the Inherits
// field cleared.
//
...
func resolveTemplate(ctx context.Context, session *messaging.Session, templateRef string, serverName string) (*schema.TemplateContent, error) {
	...
	for {
		...
		template, err := fetchTemplate(ctx, session, currentRef, serverName)
		if err != nil {
			return nil, err
		}
	...
	for i := len(chain) - 2; i >= 0; i-- {
		result = mergeTemplateContent(&result, &chain[i])
	}

	return &result, nil
}

// fetchTemplate resolves a single template reference to its content. It
// resolves the room alias to a room ID, then fetches the m.bureau.template
// state event for the template name.
func fetchTemplate(ctx context.Context, session *messaging.Session, ref schema.TemplateRef, serverName string) (*schema.TemplateContent, error) {
...
// mergeTemplateContent merges a child template over a parent template.
//
// Merge rules:
...
func mergeTemplateContent(parent, child *schema.TemplateContent) schema.TemplateContent {
	result := *parent
...
func resolveInstanceConfig(template *schema.TemplateContent, assignment *schema.PrincipalAssignment) *schema.SandboxSpec {
	...
	spec.Payload = mergeAnyMaps(template.DefaultPayload, assignment.Payload)
...
// Dest (child wins when the same destination appears in both). This matches
// the merge semantics of sandbox.MergeProfiles.
