...
func TestMergeTemplateContentMountDeduplication(t *testing.T) {
	t.Parallel()
	...
	child := &schema.TemplateContent{
		Filesystem: []schema.TemplateMount{
			{Type: "tmpfs", Dest: "/tmp", Options: "size=128M"}, // Override parent /tmp
			{Source: "${WORKTREE}", Dest: "/workspace", Mode: "rw"},
		},
	}

	result := mergeTemplateContent(parent, child)
...
func TestMergeStringSlicesDeduplication(t *testing.T) {
	...
	child := []string{"/run/bureau", "/workspace/.cache"} // /run/bureau is a duplicate

	result := mergeStringSlices(parent, child)

	// Should be 4 unique entries.
	if len(result) != 4 {
