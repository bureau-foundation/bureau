...
// CompleteLog handles the "complete-log" socket action. Flushes any
// remaining data and transitions the session to "complete". Returns
// nil if the session was already completed or doesn't exist.
func (m *logManager) CompleteLog(ctx context.Context, sourceLocalpart, sessionID string) error {
	key := sessionKey{source: sourceLocalpart, sessionID: sessionID}
	...
	if !exists {
		// Session doesn't exist — either already cleaned up or
		// never received deltas. Return success idempotently.
		return nil
	}
