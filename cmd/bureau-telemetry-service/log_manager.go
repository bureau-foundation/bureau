...
type sessionKey struct {
	source    ref.UserID // Full Matrix user ID — federation-safe, globally unique.
	sessionID string
}
...
// exist (idempotent).
func (m *logManager) CompleteLog(ctx context.Context, source ref.UserID, sessionID string) error {
	if sessionID != "" {
		return m.completeByKey(ctx, sessionKey{source: source, sessionID: sessionID})
	}
