...
of the m.bureau.agent_metrics state event.`,
		...
		Run: cli.RequireLocalpart("agent", "bureau agent metrics <localpart> [--machine <machine>]", func(ctx context.Context, localpart string, logger *slog.Logger) error {
...
func runMetrics(ctx context.Context, localpart string, logger *slog.Logger, params agentMetricsParams) error {
	...
	content, err := messaging.GetState[agentschema.AgentMetricsContent](ctx, session, location.ConfigRoomID, agentschema.EventTypeAgentMetrics, localpart)
	if err != nil {
