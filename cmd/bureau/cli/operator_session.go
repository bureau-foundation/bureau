...
func LoadSessionFrom(path string) (*OperatorSession, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return nil, NotFound("no Bureau session found at %s — run \"bureau login\" first", path)
		}
		return nil, Internal("reading session file %s: %w", path, err)
	}

	var session OperatorSession
	if err := json.Unmarshal(data, &session); err != nil {
		secret.Zero(data)
		return nil, Internal("parsing session file %s: %w", path, err)
	}
	secret.Zero(data)

	if session.UserID == "" {
		return nil, Validation("session file %s has no user_id", path)
	}
	if session.AccessToken == "" {
		return nil, Validation("session file %s has no access_token", path)
	}
	if session.Homeserver == "" {
		return nil, Validation("session file %s has no homeserver", path)
	}
...
func SaveSessionTo(session *OperatorSession, path string) error {
	...
	if err != nil {
		return Internal("marshaling session: %w", err)
	}
	data = append(data, '\n')
	...
	if err := os.MkdirAll(directory, 0700); err != nil {
		secret.Zero(data)
		return Internal("creating session directory %s: %w", directory, err)
	}

	writeError := os.WriteFile(path, data, 0600)
	secret.Zero(data)
	if writeError != nil {
		return Internal("writing session file %s: %w", path, writeError)
	}
