...
func checkTemplateUpdate(ctx context.Context, target *updateTarget, system string, logger *slog.Logger) templateUpdateResult {
	...
	var newOrigin *schema.TemplateOrigin
	var err error

	switch {
	case origin.FlakeRef != "":
		result.Source = "flake"
		result.OldRevision = shortRevision(origin.ResolvedRev)
		...
		newContent, newOrigin, err = loadFromFlake(ctx, origin.FlakeRef, system, logger)
	...
	case origin.URL != "":
		...
		newContent, newOrigin, err = loadFromURL(ctx, origin.URL, logger)
	...
	if newOrigin.ResolvedRev != "" {
		result.NewRevision = shortRevision(newOrigin.ResolvedRev)
	}
