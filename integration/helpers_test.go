...
func TestMain(m *testing.M) {
	...
	// Resolve the Nix integration-test-env store path. The nix_env Bazel
	// module extension (build/nix_env.bzl) builds the derivation during
	// Bazel's loading phase and caches the store path in a runfiles file.
	// This function reads that file — no nix invocation at test time.
	resolveRunnerEnv()
...
// writePasswordFile writes a password string to a temporary file and returns
// the file path. Used for --password-file in bureau login.
...
// resolveRunnerEnv results are set once in TestMain and read by findRunnerEnv.
var (
	runnerEnvPath     string
	runnerEnvFatalMsg string // non-empty means fatal (misconfiguration or broken derivation)
)

// resolveRunnerEnv reads the Nix integration-test-env store path from the
// Bazel-provided file. The nix_env module extension (build/nix_env.bzl) runs
// `nix build` during Bazel's loading phase — where network access is expected
// — and caches the store path. This function reads that cached result. No nix
// invocation happens at test time.
//
// Called once from TestMain, before any tests start.
func resolveRunnerEnv() {
	// INTEGRATION_TEST_ENV is an rlocationpath to the store_path file
	// generated by the nix_env module extension. Resolve it against
	// RUNFILES_DIR the same way testutil.DataBinary does for binary paths.
	envPathRelative := os.Getenv("INTEGRATION_TEST_ENV")
	if envPathRelative == "" {
		runnerEnvFatalMsg = "INTEGRATION_TEST_ENV not set (run tests via: bazel test //integration:integration_test)"
		return
	}

	runfilesDirectory := os.Getenv("RUNFILES_DIR")
	if runfilesDirectory == "" {
		runnerEnvFatalMsg = "RUNFILES_DIR not set (run tests via: bazel test //integration:integration_test)"
		return
	}

	storePathFile := filepath.Join(runfilesDirectory, envPathRelative)
	data, err := os.ReadFile(storePathFile)
	if err != nil {
		runnerEnvFatalMsg = fmt.Sprintf("read integration test env path from %s: %v", storePathFile, err)
		return
	}

	storePath := strings.TrimSpace(string(data))
	if storePath == "" {
		runnerEnvFatalMsg = "integration test env store path file is empty"
		return
	}

	// Verify the store path exists on disk with the expected bin/sh.
	// The nix_env extension built the derivation, so the store path
	// should exist. If it doesn't, the nix store was garbage-collected
	// after the extension ran — rebuild with `bazel fetch @integration_test_env`.
	binDirectory := filepath.Join(storePath, "bin")
	if _, err := os.Stat(binDirectory); err != nil {
		runnerEnvFatalMsg = fmt.Sprintf(
			"integration-test-env not found at %s (nix store may have been garbage-collected).\n"+
				"Rebuild with: bazel fetch @integration_test_env//...",
			storePath)
		return
	}
	if _, err := os.Stat(filepath.Join(binDirectory, "sh")); err != nil {
		runnerEnvFatalMsg = fmt.Sprintf("integration-test-env missing sh at %s: %v", binDirectory, err)
		return
	}
...
// findRunnerEnv returns the Nix integration-test-env store path, or fails
// the test if the environment is not available. The store path is resolved
// once in TestMain by resolveRunnerEnv; this function reads the cached result.
