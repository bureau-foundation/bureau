...
// based [Store] that does not participate in ring eviction.
type Cache struct {
	config   CacheConfig
	device   *CacheDevice
	ring     *BlockRing
...
func NewCache(config CacheConfig) (*Cache, error) {
	...
	pinPath := filepath.Join(config.Path, "pinned")
	pinStore, err := NewStore(pinPath)
	if err != nil {
		return nil, fmt.Errorf("creating pin store: %w", err)
	}
...
func (c *Cache) Get(containerHash Hash) ([]byte, error) {
	// Check pin store first (pinned containers are never evicted).
	if c.pinStore.Exists(containerHash) {
		return c.pinStore.ReadContent(containerHash)
	}
...
// Pin stores a container in the pin store, exempt from ring eviction.
// Pinned containers persist until explicitly unpinned.
func (c *Cache) Pin(containerHash Hash, data []byte) error {
	c.writeMu.Lock()
	defer c.writeMu.Unlock()

	_, err := c.pinStore.WriteContent(data, "")
	return err
}

// Unpin removes a container from the pin store. The container may
// still exist in the ring cache if it was also Put there.
func (c *Cache) Unpin(containerHash Hash) error {
	// The pin store uses file-hash-addressed reconstruction records.
	// Pinning stores whole containers as artifacts in the pin store.
	// To unpin, we'd need to delete the reconstruction record and
	// container file. For now, this is a placeholder — the full
	// unpin implementation requires a Store.Delete method.
	return fmt.Errorf("unpin not yet implemented")
}
