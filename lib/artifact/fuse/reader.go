...
type containerProvider interface {
	...
	containerBytes(hash artifact.Hash) ([]byte, error)
}
...
func (p *storeProvider) containerBytes(hash artifact.Hash) ([]byte, error) {
	// Check cache first.
	if p.cache != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("reading container %s: %w", artifact.FormatHash(hash), err)
	}
...
func buildChunkTable(record *artifact.ReconstructionRecord, provider containerProvider) ([]chunkEntry, error) {
	...
	for segmentIndex, segment := range record.Segments {
		...
		cr, err := artifact.ReadContainerIndex(bytes.NewReader(containerData))
		if err != nil {
			return nil, fmt.Errorf("segment %d: reading container index for %s: %w",
				segmentIndex, artifact.FormatHash(segment.Container), err)
		}
...
func extractChunkData(provider containerProvider, chunk chunkEntry) ([]byte, error) {
	...
	cr, err := artifact.ReadContainerIndex(reader)
	if err != nil {
		return nil, fmt.Errorf("parsing container %s: %w",
			artifact.FormatHash(chunk.container), err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("extracting chunk %d from container %s: %w",
			chunk.indexInContainer, artifact.FormatHash(chunk.container), err)
	}
