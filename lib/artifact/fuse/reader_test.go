...
type testProvider struct {
	containers map[artifact.Hash][]byte
}

func (p *testProvider) containerBytes(hash artifact.Hash) ([]byte, error) {
...
func (e *containerNotFoundError) Error() string {
	return "container not found: " + artifact.FormatHash(e.hash)
}

// storeAndBuildProvider stores content in a real Store and wraps it
// in a storeProvider (no cache).
func storeAndBuildProvider(t *testing.T, content []byte, contentType string) (*artifact.Store, *artifact.StoreResult, containerProvider) {
	t.Helper()
	store, err := artifact.NewStore(filepath.Join(t.TempDir(), "artifact"))
	if err != nil {
