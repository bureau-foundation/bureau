...
func TestIndex_SetAndRemovePrincipal(t *testing.T) {
	...
	policy := schema.AuthorizationPolicy{
		Grants: []schema.Grant{
			{Actions: []string{observation.ActionAll}, Targets: []string{"bureau/dev/**:bureau.local"}},
		},
	...
	if grants[0].Actions[0] != observation.ActionAll {
		t.Errorf("grant action = %q, want %s", grants[0].Actions[0], observation.ActionAll)
	}
...
func TestIndex_RemovePrincipalCleansTemporalGrants(t *testing.T) {
	...
	index.SetPrincipal(uid(t, "@agent:bureau.local"), schema.AuthorizationPolicy{
		Grants: []schema.Grant{{Actions: []string{ticket.ActionCreate}}},
	})
...
func TestIndex_SweepExpired(t *testing.T) {
	...
	index.AddTemporalGrant(uid(t, "@agent-a:bureau.local"), schema.Grant{
		Actions: []string{observation.ActionObserve}, Targets: []string{"**:**"},
...
func TestIndex_SweepExpired_PreservesStaticGrants(t *testing.T) {
	...
	index.SetPrincipal(uid(t, "@agent:bureau.local"), schema.AuthorizationPolicy{
		Grants: []schema.Grant{
			{Actions: []string{ticket.ActionCreate}},
			{Actions: []string{observation.ActionObserve}, ExpiresAt: expiry.Format(time.RFC3339)},
		},
	})
	...
	for _, grant := range grants {
		if grant.ExpiresAt == expiry.Format(time.RFC3339) && len(grant.Actions) == 1 && grant.Actions[0] == observation.ActionObserve {
...
func TestIndex_GrantsCopy(t *testing.T) {
	index := NewIndex()
	index.SetPrincipal(uid(t, "@agent:bureau.local"), schema.AuthorizationPolicy{
		Grants: []schema.Grant{{Actions: []string{observation.ActionObserve}}},
	})
	...
	if grants2[0].Actions[0] != observation.ActionObserve {
...
func TestIndex_AllowancesCopy(t *testing.T) {
	index := NewIndex()
	index.SetPrincipal(uid(t, "@agent:bureau.local"), schema.AuthorizationPolicy{
		Allowances: []schema.Allowance{{Actions: []string{observation.ActionObserve}, Actors: []string{"admin:bureau.local"}}},
	})
	...
	if allowances2[0].Actions[0] != observation.ActionObserve {
...
func TestIndex_DenialsAccessor(t *testing.T) {
	...
	policy := schema.AuthorizationPolicy{
		...
		Denials: []schema.Denial{
			{Actions: []string{observation.ActionReadWrite}, Targets: []string{"secret/**:bureau.local"}},
	...
	if denials[0].Actions[0] != observation.ActionReadWrite {
		t.Errorf("denial[0] action = %q, want %s", denials[0].Actions[0], observation.ActionReadWrite)
	}
	...
	if original[0].Actions[0] != observation.ActionReadWrite {
...
func TestIndex_AllowanceDenialsAccessor(t *testing.T) {
	...
	policy := schema.AuthorizationPolicy{
		Allowances: []schema.Allowance{
			{Actions: []string{observation.ActionAll}, Actors: []string{"**:**"}},
		},
		AllowanceDenials: []schema.AllowanceDenial{
			{Actions: []string{observation.ActionReadWrite}, Actors: []string{"untrusted/**:bureau.local"}},
		},
	}
	...
	if allowanceDenials[0].Actions[0] != observation.ActionReadWrite {
		t.Errorf("allowance denial action = %q, want %s", allowanceDenials[0].Actions[0], observation.ActionReadWrite)
	}
