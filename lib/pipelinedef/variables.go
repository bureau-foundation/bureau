...
func ResolveVariables(declarations map[string]pipeline.PipelineVariable, payload map[string]string, environ func(string) string) (map[string]string, error) {
...
func ExpandStep(step pipeline.PipelineStep, variables map[string]string) (pipeline.PipelineStep, error) {
	...
	if len(step.Env) > 0 {
		...
		for name, value := range step.Env {
			...
			if err != nil {
				return pipeline.PipelineStep{}, fmt.Errorf("step %q env[%s]: %w", step.Name, name, err)
			}
	...
	if step.Run, err = Expand(step.Run, merged); err != nil {
		return pipeline.PipelineStep{}, fmt.Errorf("step %q run: %w", step.Name, err)
	}
	if step.Check, err = Expand(step.Check, merged); err != nil {
		return pipeline.PipelineStep{}, fmt.Errorf("step %q check: %w", step.Name, err)
	}
	if step.When, err = Expand(step.When, merged); err != nil {
		return pipeline.PipelineStep{}, fmt.Errorf("step %q when: %w", step.Name, err)
	}
	...
	if step.Publish != nil {
		...
		if expandedEventType, expandErr := Expand(string(expanded.EventType), merged); expandErr != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q publish.event_type: %w", step.Name, expandErr)
		} else {
			expanded.EventType = ref.EventType(expandedEventType)
		}
		if expanded.Room, err = Expand(expanded.Room, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q publish.room: %w", step.Name, err)
		}
		if expanded.StateKey, err = Expand(expanded.StateKey, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q publish.state_key: %w", step.Name, err)
		}
		if expanded.Content, err = expandMap(expanded.Content, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q publish.content: %w", step.Name, err)
		}
	...
	if step.AssertState != nil {
		...
		if expanded.Room, err = Expand(expanded.Room, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.room: %w", step.Name, err)
		}
		if expandedEventType, expandErr := Expand(string(expanded.EventType), merged); expandErr != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.event_type: %w", step.Name, expandErr)
		} else {
			expanded.EventType = ref.EventType(expandedEventType)
		}
		if expanded.StateKey, err = Expand(expanded.StateKey, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.state_key: %w", step.Name, err)
		}
		if expanded.Field, err = Expand(expanded.Field, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.field: %w", step.Name, err)
		}
		if expanded.Equals, err = Expand(expanded.Equals, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.equals: %w", step.Name, err)
		}
		if expanded.NotEquals, err = Expand(expanded.NotEquals, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.not_equals: %w", step.Name, err)
		}
		if expanded.Message, err = Expand(expanded.Message, merged); err != nil {
			return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.message: %w", step.Name, err)
		}
		if len(expanded.In) > 0 {
			...
			for index, value := range expanded.In {
				if expandedIn[index], err = Expand(value, merged); err != nil {
					return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.in[%d]: %w", step.Name, index, err)
				}
			}
		...
		if len(expanded.NotIn) > 0 {
			...
			for index, value := range expanded.NotIn {
				if expandedNotIn[index], err = Expand(value, merged); err != nil {
					return pipeline.PipelineStep{}, fmt.Errorf("step %q assert_state.not_in[%d]: %w", step.Name, index, err)
				}
			}
	...
	if len(step.Outputs) > 0 {
		...
		for name, raw := range step.Outputs {
			parsed, parseErr := pipeline.ParseStepOutputs(map[string]json.RawMessage{name: raw})
			if parseErr != nil {
				return pipeline.PipelineStep{}, fmt.Errorf("step %q outputs[%s]: %w", step.Name, name, parseErr)
			}
			output := parsed[name]
			if output.Path, err = Expand(output.Path, merged); err != nil {
				return pipeline.PipelineStep{}, fmt.Errorf("step %q outputs[%s].path: %w", step.Name, name, err)
			}
			serialized, marshalErr := json.Marshal(output)
			if marshalErr != nil {
				return pipeline.PipelineStep{}, fmt.Errorf("step %q outputs[%s]: re-serializing expanded output: %w", step.Name, name, marshalErr)
			}
