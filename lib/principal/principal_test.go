...
func TestParseFleetPrefix(t *testing.T) {
	tests := []struct {
		name          string
		prefix        string
		wantNamespace string
		...
		{
			name:          "bureau prod",
			prefix:        "bureau/fleet/prod",
			wantNamespace: "bureau",
			wantFleetName: "prod",
		},
		{
			name:          "acme staging",
			prefix:        "acme/fleet/staging",
			wantNamespace: "acme",
			wantFleetName: "staging",
		},
		{
			name:          "hyphenated fleet name",
			prefix:        "bureau/fleet/us-east-gpu",
			wantNamespace: "bureau",
			wantFleetName: "us-east-gpu",
		},
		{
			name:      "no fleet separator",
			prefix:    "bureau/machine/gpu",
			wantError: true,
		},
		{
			name:      "empty namespace",
		...
		},
		{
			...
			prefix:    "bureau/fleet/prod/machine",
			wantError: true,
		},
		{
			name:      "empty string",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			namespace, fleetName, err := ParseFleetPrefix(test.prefix)
			if test.wantError {
				if err == nil {
					t.Errorf("ParseFleetPrefix(%q) = (%q, %q, nil), want error", test.prefix, namespace, fleetName)
				}
				return
			}
			if err != nil {
				t.Fatalf("ParseFleetPrefix(%q) error: %v", test.prefix, err)
			}
			if namespace != test.wantNamespace {
				t.Errorf("ParseFleetPrefix(%q) namespace = %q, want %q", test.prefix, namespace, test.wantNamespace)
			}
			if fleetName != test.wantFleetName {
				t.Errorf("ParseFleetPrefix(%q) fleetName = %q, want %q", test.prefix, fleetName, test.wantFleetName)
			}
		})
	}
}

func TestParseFleetPrefix_RoundTrip(t *testing.T) {
	// FleetPrefix → ParseFleetPrefix should round-trip.
	namespace, fleetName := "bureau", "prod"
	prefix := FleetPrefix(namespace, fleetName)
	gotNamespace, gotFleetName, err := ParseFleetPrefix(prefix)
	if err != nil {
		t.Fatalf("ParseFleetPrefix(%q) error: %v", prefix, err)
	}
	if gotNamespace != namespace {
		t.Errorf("round-trip namespace: got %q, want %q", gotNamespace, namespace)
	}
	if gotFleetName != fleetName {
		t.Errorf("round-trip fleetName: got %q, want %q", gotFleetName, fleetName)
	}
}
