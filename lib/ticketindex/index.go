...
type Index struct {
	tickets map[string]ticket.TicketContent
...
func NewIndex() *Index {
	return &Index{
		tickets:    make(map[string]ticket.TicketContent),
...
func (idx *Index) Put(ticketID string, content ticket.TicketContent) {
	...
	if len(content.Gates) > 0 {
		cloned := make([]ticket.TicketGate, len(content.Gates))
		copy(cloned, content.Gates)
		content.Gates = cloned
	}
	if len(content.Notes) > 0 {
		cloned := make([]ticket.TicketNote, len(content.Notes))
		copy(cloned, content.Notes)
		content.Notes = cloned
	}
	if len(content.Attachments) > 0 {
		cloned := make([]ticket.TicketAttachment, len(content.Attachments))
...
func (idx *Index) Get(ticketID string) (ticket.TicketContent, bool) {
	content, exists := idx.tickets[ticketID]
	return content, exists
}

// Ready returns all actionable tickets: those available to be picked
// up (status "open", all blockers closed, all gates satisfied), those
// already being worked on (status "in_progress"), and those waiting
// for reviewer feedback (status "review"). Results are sorted by
// priority (ascending, 0=critical first) then by creation time
// (oldest first).
...
func (idx *Index) EpicHealth(epicID string) ticket.EpicHealthStats {
	childIDs, exists := idx.children[epicID]
	if !exists {
		return ticket.EpicHealthStats{}
	}

	var stats ticket.EpicHealthStats
	for childID := range childIDs {
		content, exists := idx.tickets[childID]
		if !exists {
			continue
...
// Composite formula.
func (idx *Index) Score(ticketID string, now time.Time, weights RankWeights) ticket.TicketScore {
	weights = weights.applyDefaults()

	content, exists := idx.tickets[ticketID]
	if !exists {
		return ticket.TicketScore{BorrowedPriority: -1}
	}
...
func readySince(content *ticket.TicketContent, idx *Index) string {
	if len(content.BlockedBy) == 0 {
...
func (idx *Index) matchesFilter(content *ticket.TicketContent, filter *Filter) bool {
	if filter.Status != "" {
...
func grepMatches(re *regexp.Regexp, content *ticket.TicketContent) bool {
