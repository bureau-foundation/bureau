...
// NoteModal is a modal overlay for adding a note to a ticket. It
// implements a simple multi-line text editor with cursor tracking,
// rendered as a centered overlay on top of the main view.
...
// NewNoteModal creates a NoteModal for adding a note to the given
// ticket. The textarea starts empty and focused.
func NewNoteModal(ticketID string, theme Theme) NoteModal {
	return NoteModal{
		TicketID: ticketID,
		lines:    [][]rune{{}},
		cursorY:  0,
...
func (modal NoteModal) Render(screenWidth, screenHeight int) ([]string, int, int) {
	...
	// Build title line.
	title := titleStyle.Render("Add Note to " + modal.TicketID)
